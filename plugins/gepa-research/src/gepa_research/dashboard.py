...
from .gepa_adapter import progress_path
...
def create_app(root: Path | None = None) -> Flask:
    ...
    app.config["GEPA_RESEARCH_ROOT"] = str(root or repo_root())

    def _root() -> Path:
        return Path(app.config["GEPA_RESEARCH_ROOT"])
    ...
    def progress():
        return jsonify(load_json(progress_path(_root()), {}))
    ...
    def activate_run(run_id: str):
        run_dir = gepa_research_dir(_root()) / run_id
...
def main() -> None:
    import os
    port = int(os.environ.get("GEPA_RESEARCH_DASHBOARD_PORT", "8080"))
    app = create_app()
