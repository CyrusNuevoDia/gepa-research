...
**All construction happens inside the baseline worktree (experiment 0), not in main.** Main stays free of gepa-research-specific infrastructure. The constructed benchmark becomes part of the baseline commit; all descendant experiments inherit it automatically.
...
The core risk of a constructed benchmark: *gepa-research will optimize the metric, not the underlying thing you care about*. Every constructed benchmark needs an explicit Goodhart audit before being committed.

Ask these questions:

1. **Can the metric go up without the underlying thing improving?** Think of at least one degenerate way to game it (e.g., "special-case the exact inputs in the test set", "output a constant that averages well", "return early with a trivial answer"). If you can think of gaming strategies, gepa-research will find them faster.
...
3. **What's the held-out signal that would catch gaming?** If gepa-research gets a suspiciously high score, what independent test would reveal the gaming?

Document the answers in `.gepa-research/project.md` under a "Benchmark gaming risks" section, and mitigate each with either:
...
- **Training slice** (what gepa-research sees): 60-70% of cases. This is the score gepa-research optimizes.
- **Held-out slice** (what gepa-research can't see): 30-40% of cases. Evaluated separately after each committed experiment.

If the training score improves but the held-out score doesn't (or regresses), gepa-research is overfitting to the specific training cases. The held-out score becomes either a gate or an annotation that the orchestrator watches.

For the initial v0.1.0 flow, implement the held-out slice as a **gate** (next section). For a later iteration, add first-class support in the gepa-research CLI for "hidden eval" scoring that's orthogonal to the optimization score.
...
Add the gate via `gepa-research gate add root --name <name> --command <command>` during the discover flow. The gate runs alongside every experiment and auto-discards any experiment that breaks it, even if the benchmark score improves.

**The gate command must exit non-zero on regression.** `gepa-research run` checks exit code, not stdout. A bare `python3 benchmark.py --task-ids 5,6,9` always exits 0 because the benchmark script's contract is "exit 0 unless infrastructure broke" -- it prints a low score but never fails. To make a benchmark-derived gate actually catch regressions, the benchmark needs a `--min-score <threshold>` flag (or equivalent) that:
...
Running `gepa-research run <exp_id>` captures the baseline score and marks the experiment committed in a single step. No separate `gepa-research done` is needed.

## Rollback

If the Goodhart check flags an unmitigable gaming risk, or the benchmark doesn't run cleanly, discard experiment 0 with `gepa-research discard <exp_id>` and restart from root with a simpler benchmark.
...
- v0.1.0 of gepa-research compares experiment scores directly (single sample per experiment). It does **not** yet retry experiments, average across runs, or use confidence intervals. A noisy benchmark can therefore commit a "lucky" experiment as a real improvement.
...
Record what you know about the benchmark's determinism in `.gepa-research/project.md`, one line under "Benchmark determinism":
