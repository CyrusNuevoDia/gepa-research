...
class Backend(Protocol):
    """Transport layer between the SDK and the gepa-research CLI (or a remote server).
...
class LocalBackend:
    """Writes trace files to ``$GEPA_RESEARCH_TRACES_DIR`` and result JSON to
    ``$GEPA_RESEARCH_RESULT_PATH`` (or stdout in legacy mode).
    ...
    def emit_result(self, result: dict[str, Any]) -> None:
        ...
        result_path = os.environ.get("GEPA_RESEARCH_RESULT_PATH")
        if not result_path:
            print(payload, file=self._stdout)
            return
        target = Path(result_path)
        target.parent.mkdir(parents=True, exist_ok=True)
        # Claim destination + tmp+rename: a second writer (e.g. a stray
        # benchmark-derived gate) fails fast on the O_EXCL claim; a crash
        # mid-publish leaves an empty file at target (caught by load_result
        # in the runner) instead of a partial write. Same-instance double
        # calls to Run.finish() are guarded upstream in Run._finished.
        try:
            os.close(os.open(target, os.O_CREAT | os.O_EXCL | os.O_WRONLY))
        except FileExistsError:
            raise RuntimeError(
                f"{target} already exists; only one writer per attempt"
            ) from None
...
def default_backend() -> Backend:
    ...
    server = os.environ.get("GEPA_RESEARCH_SERVER")
    if server:
        raise NotImplementedError(
            f"HTTP backend not yet available (GEPA_RESEARCH_SERVER={server}). "
