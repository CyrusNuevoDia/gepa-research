...
def gepa_research_dir(root: Path) -> Path:
    """Top-level .gepa-research/ container."""
    return root / WORKSPACE_NAME


def _meta_path(root: Path) -> Path:
    return gepa_research_dir(root) / META_FILE
...
def workspace_path(root: Path) -> Path:
    """Path to the active run directory (e.g. .gepa-research/run_0000/)."""
    meta = _load_meta(root)
    active = meta.get("active")
    if active:
        return gepa_research_dir(root) / active
    if (gepa_research_dir(root) / CONFIG_FILE).exists():
        return gepa_research_dir(root)
    return gepa_research_dir(root)
...
def project_path(root: Path) -> Path:
    ...
    return gepa_research_dir(root) / PROJECT_FILE
...
def list_runs(root: Path) -> list[dict[str, Any]]:
    ...
    active = meta.get("active")
    runs = []
    ws_root = gepa_research_dir(root)
...
def attempt_result_path(root: Path, exp_id: str, attempt: int) -> Path:
    # Where the benchmark drops its score JSON via $GEPA_RESEARCH_RESULT_PATH.
    # Distinct from experiment_result_path() (one level up); both are named
    # "result.json" but live at different levels of the experiment tree.
    return attempt_dir(root, exp_id, attempt) / "result.json"
...
def experiment_result_path(root: Path, exp_id: str) -> Path:
    # Overwritten on every run; reflects only the latest attempt.
...
def allocate_experiment(root: Path, parent_id: str, hypothesis: str) -> dict[str, Any]:
    ...
    with advisory_lock(lock_file_for(gpath)):
        ...
        branch = f"gepa-research/{run_id}/{exp_id}"
        ...
        if project_src.exists():
            worktree_ws = worktree / WORKSPACE_NAME
            worktree_ws.mkdir(parents=True, exist_ok=True)
            shutil.copy2(str(project_src), str(worktree_ws / PROJECT_FILE))
...
def reset_runtime_state(root: Path) -> None:
    ...
    subprocess.run(["git", "worktree", "prune"], cwd=root, check=False)
    # Only delete branches for this run (gepa-research/<run_id>/*)
    branch_prefix = f"refs/heads/gepa-research/{run_id}/" if run_id else "refs/heads/gepa-research/"
    result = subprocess.run(
...
def maybe_commit_worktree(node: dict[str, Any], hypothesis: str) -> str | None:
    ...
    subprocess.run(
        ["git", "commit", "-m", f"gepa-research: {node['id']} {hypothesis}"],
        cwd=worktree,
        check=True,
    )
    return current_commit(worktree)
...
def resolve_parent_score(graph: dict[str, Any], parent_id: str) -> float | None:
    if parent_id == "root":
        return None
    return graph["nodes"][parent_id].get("score")
