...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    # Persist optional GEPA-adapter fields into config.json so `optimize` can read them.
    if args.objective or args.background or args.reflection_lm or args.num_parallel_proposals:
        cfg = load_config(root)
        if args.objective:
            cfg["optimization_objective"] = args.objective
        if args.background:
            cfg["background"] = args.background
        if args.reflection_lm:
            cfg["reflection_lm"] = args.reflection_lm
        if args.num_parallel_proposals:
            cfg["num_parallel_proposals"] = int(args.num_parallel_proposals)
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if evaluated_attempts >= max_attempts:
        print(
            ...
            f"Discard with `gepa-research discard {args.exp_id} --reason \"...\"` or branch elsewhere.",
            file=sys.stderr,
        )
        return 1
    ...
    env["GEPA_RESEARCH_EXPERIMENT_ID"] = args.exp_id
...
def cmd_optimize(args: argparse.Namespace) -> int:
    ...
    parent_id = best["id"]
    num_parallel = args.num_parallel_proposals
    ...
    print(
        ...
        f"max-metric-calls={args.max_metric_calls} stall={args.stall} "
    ...
    # Surface the key fields so the skill's final summary is accurate.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gepa-research")
    parser.add_argument(
        "--version",
        action="version",
    ...
    init_p.add_argument("--objective", default=None, help="Natural-language goal passed to GEPA's reflection LM.")
    init_p.add_argument("--background", default=None, help="Domain knowledge / constraints passed to GEPA's reflection LM.")
    init_p.add_argument("--reflection-lm", default=None, help="Default reflection LM (e.g. anthropic/claude-opus-4-7).")
    init_p.add_argument(
        "--num-parallel-proposals",
        type=int,
        default=None,
        help="Default candidate-level concurrency for `optimize` (evaluated in separate worktrees).",
    )
    ...
    optimize_p.add_argument("--max-metric-calls", type=int, default=50)
    optimize_p.add_argument("--stall", type=int, default=5)
    optimize_p.add_argument(
        "--num-parallel-proposals",
        type=int,
        default=None,
        help="Evaluate N candidates per GEPA iteration concurrently, each in its own worktree. Default: config.json value or 1.",
    )
    optimize_p.add_argument("--reflection-lm", default=None)
    optimize_p.add_argument("--objective", default=None)
    optimize_p.add_argument("--background", default=None)
