...
function bestCandidateId() {
  ...
  for (const n of Object.values(state.graph.nodes)) {
    ...
    if (isMax ? n.score > best.score : n.score < best.score) best = n;
  }
...
async function fetchAll() {
  try {
    const [stats, graph, runs, progress] = await Promise.all([
...
// ─── Render: Progress (BUDGET + STALL) ───────────────────
function renderProgress() {
  const p = state.progress || {};
  const budgetEl = document.getElementById('budget-count');
  const budgetFill = document.getElementById('budget-bar-fill');
  const budgetDetail = document.getElementById('budget-detail');
  const stallEl = document.getElementById('stall-count');
  const stallFill = document.getElementById('stall-bar-fill');
  const stallDetail = document.getElementById('stall-detail');

  if (p.max_metric_calls == null) {
    budgetEl.textContent = '--';
    budgetFill.style.width = '0%';
    budgetDetail.textContent = 'no active run';
  } else {
    const used = p.metric_calls_used || 0;
    const max = p.max_metric_calls;
  ...
  if (p.stall_limit == null) {
    stallEl.textContent = '--';
    stallFill.style.width = '0%';
    stallDetail.textContent = 'iters since improvement';
  } else {
    const stalled = p.iterations_without_improvement || 0;
    const limit = p.stall_limit;
    stallEl.textContent = `${stalled} / ${limit}`;
    const pctStalled = limit > 0 ? Math.min(100, (stalled / limit) * 100) : 0;
    stallFill.style.width = pctStalled + '%';
    stallDetail.textContent = stalled >= limit ? 'stall limit reached' : 'iters since improvement';
  }
}
    ...
    options: {
      ...
      scales: {
        x: {
          ...
          offset: true,
          title: { display: true, text: 'candidate #', color: '#52525b', font: { size: 11 } },
          grid: { color: '#1e1e22' },
          ticks: {
            color: '#71717a',
...
function renderTable() {
  ...
  tbody.innerHTML = pageExps.map(n => {
    ...
    const bestBadge = n.id === bestId ? '<span class="best-star" title="Best candidate">★</span>' : '';
    return `<div class="table-row ${rowClass} ${rowStatusClass}" onclick="openDrawer('${n.id}')">
      <span class="col-id">${bestBadge}${shortId(n.id)}</span>
